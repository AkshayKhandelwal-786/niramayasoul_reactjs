...
function DoctorListing() {
  ...
  const { specialityId } = location.state || {}; // Retrieve the state passed from navigate
  ...
  const getDoctors = async () => {
    try {
      const response = await axiosConfig.get(`/specialities/${specialityId}`);
          ...
          <div className="row">
            <div className="col-lg-12 mx-auto">
              <div className="heading text-left mb-4 position-relative headingcart">
                <h2 className="pb-0">Doctor Listings ({doctors.length}) </h2>
              </div>
            </div>
          </div>
