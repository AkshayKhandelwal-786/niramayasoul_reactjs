...
function MedicinesList() {
  ...
  useEffect(() => {
    getProductsBasedOnCategory(page, true); // Load new page when page changes
  }, [page]);
  ...
  const updateSortedAndFilteredMedicines = () => {
    ...
    if (selectedBrand) {
      sortedArray = sortedArray.filter(
        (medicine) => medicine.brand.name === selectedBrand
      );
    }
  ...
  }, [sortOption, selectedBrand, medicines]);

  useEffect(() => {
    // Extract unique brands only when medicines are fetched
    if (medicines.length > 0) {
      const uniqueBrands = [
        ...new Set(medicines.map((medicine) => medicine.brand.name)),
      ];
      setBrands(uniqueBrands);
    }
  }, [medicines]);

  const getProductsBasedOnCategory = async (page, append = true) => {
    try {
      const response = await axiosConfig.get(`/medicine_categories/${catID}`, {
        params: {
          page: page, // Request the correct page number
          query: "", // Pass an empty query for now
        },
      });
      ...
      if (append) {
        ...
        // Replace the medicines list when loading initially or applying filter/sort
  ...
  const handleFilterChange = (e) => {
    ...
    getProductsBasedOnCategory(1, true); // Fetch the filtered medicines without appending
  };

  const handleSortChange = (e) => {
    setSortOption(e.target.value);
    setPage(1); // Reset to the first page when sorting
    getProductsBasedOnCategory(1, true); // Fetch the sorted medicines without appending
  };
              ...
              <div className="row gy-lg-0 gy-4">
                <div className="col-lg-12">
                  <div className="d-lg-flex align-items-center position-relative justify-content-end">
                    <span className="sortby">Shop by Brand</span>
                    <div className="position-relative w-100 w-17">
                      <img
                        className="dropdown-search"
                        src="/img/arrowdown.svg"
                      ...
                      <select
                        className="form-control form-cus"
                        onChange={handleFilterChange} // Call handleFilterChange on brand selection
                      >
        ...
        <div className="container">
          ...
          <div className="row">
            <div className="col-lg-12 text-center">
              <button className="btn-more btn-load-more" onClick={loadMoreMedicines}>
                Load More
              </button>
            </div>
          </div>
